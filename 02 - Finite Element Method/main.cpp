...
int main(int argc, char** argv){
    ...
    DS<float> *T, *T_full, *T_N, *M, *K, *b1, *b2, *H;
    DS<int> *dirichlet_indices, *neumann_indices;

    DS<DS<float>*> *Result,*M_locals,*K_locals,*b1_locals,*b2_locals,*H_locals;
    ...
    Math::init(T, G->get_parameter(INITIAL_A));
    ...
    while( t <= tf ){
        ...
        SDDS<DS<float>*>::create(&b1_locals, nelems, ARRAY);
        SDDS<DS<float>*>::create(&b2_locals, nelems, ARRAY);
        ...
        for(int e = 0; e < nelems; e++){
            ...
            //Se calcula la b1 local y se añade al listado de matrices b. Se envía la fuente de calor
        ...
        SDDS<float>::create(&b1, nnodes, 1, MATRIX);      Math::zeroes(b1);
        SDDS<float>::create(&b2, nnodes, 1, MATRIX);      Math::zeroes(b2);
        ...
        FEM::apply_Dirichlet(nnodes, free_nodes, &b1, K, Td, dirichlet_indices);
        FEM::apply_Dirichlet(nnodes, free_nodes, &b2, K, Td, dirichlet_indices);
